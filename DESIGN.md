...
**批量写红线（v0.4.0 补）**：持锁期间**不得逐行提交**——WAL 下每次 `commit`
一次 fsync，N 行 = 世界冻结 N×fsync（几千格的地图复制会把 tick/移动/所有玩家
卡住数秒）。批量路径一律走 `store.save_locations` / `save_entities`
（单事务 `executemany`），单条写退化为单元素调用。
...
- **maps**：增加 `world_id / folder_id(NULL=世界根) / sort`——**文件夹与地图共用
  一个序号空间**（"同级排序"是同一容器内的全序，见 §4.3.2）
...
- **分发按实体所在世界过滤（v0.3.0 全路径落地）**——未激活包在该世界**整体
  视为不存在**：
  | 面 | 过滤点 | 未激活时的表现 |
  |---|---|---|
  | MCP 工具 | 动态工具入口按**调用者**世界 | 明确拒绝（"该世界未启用该玩法包"） |
  | 玩家视图 | `GET /views` 按调用者世界 | 该包的 tab 不出现 |
  | 交互动作 | `engine.interact` / 可用动作菜单按目标世界 | 动作不可用、菜单里不列 |
  | 事件订阅 | `_binding_active`（on_tick 例外） | 事件不触发 |
  | 原语覆盖/过滤器 | `_dispatch_locked` 按**行为主体**世界 | 回落内核默认实现 |
  | kind 声明 | `block_move` / 交互按实体世界 | 不挡路、动作不出现 |
  | 部件 UI（ui_hook） | 展开时按**查看者**世界 | 注入方不在该世界 → 不注入 |
- **未归属世界的地图**按**默认世界**（`default`）的激活集合算——"没有归属"
  不等于"没有规则"；管理端建图即归属当前世界，并提供孤儿地图一键归属
- 切换即时生效：给某世界停用一个包 = 该世界失去对应行为，其他世界不受影响
- **仍为全局**：跨包服务（服务是包与包之间的调用约定，不是世界内能力）、
  物品定义等注册表本身（实例按世界过滤的是"能力面"）；世界间的数据可见性
  （`/state` 返回哪些地图）与出生点选择见 GAPS G15 遗留项
...
### 3.1 实体（字段化 + 标签组合，D9 / D10 / D18）

实体只保留**最小身份与位置**，一切数据以**字段**承载（容器为 `attrs` 与
`state` 两个 dict）；**能力由标签承载**——基底 `kind` 是"预设的标志位"，
实例级 `tags[]` 与之叠加，能力取并集（D18/D19）。

```python
@dataclass
class Entity:
    id: str          # uuid4 hex
    kind: str        # 基底类型（player = 身份类型；或玩法包注册的名字）
    map_id: str      # 位置
    row: int
    col: int
    name: str        # 显示主元素
    ...
    tags: list[str] = []         # 实例级标签（有序，参与能力并集与字段覆盖顺序）
```

**能力解析（D18/D19/D21）**：

```
有效标签 = 基底 kind + 类型预设标签（register_entity_type(kind, tags=[...])）+ 实例 tags
能力     = ∪ 各标签的声明（**逐标签按世界激活过滤**，D21；未注册标签不贡献）
block_move   = 任一标签为真即阻挡（state["block_move"] 仍是最高优先的动态覆盖）
interactions = 并集
fields       = 隐式 kind 标签 → 实例 tags（数组序）→ 分类字段（越实例化越优先）
```

- **类型与标签共用一个命名空间**：`register_entity_kind("door", ...)` 就是注册一条
  名为 `door` 的**隐式标签**（`implicit=True`）——所以旧包、旧数据零改动，
  而 `add_tag_fields("monster", ...)` 自然变成"给**所有带 monster 标签**的实体加字段"
- **`register_entity_tag(tag, ...)`**：显式声明一条可叠加的能力标签
  （"给玩家再组合一个刷怪笼标签" = 玩家的基底 kind 不变，能力两边都有）
- **身份只看基底 kind**（D20）：标签里写 `player` 不改变身份，
  `is_identity()` 也不看标签——所以不存在"删标签绕过 D14 保护"的路径
- 规格级能力缓存以 `(kind, tuple(tags))` 为键，包加载/卸载时清空（D21）

**三层次字段**：

| 层次 | API | 用途 |
|---|---|---|
| 标签/类型声明字段 | `register_entity_kind(kind, ..., fields=[{name,label,type,default?}])` / `register_entity_tag` | 声明级 schema，UI 通用渲染（角色卡/编辑表单）；类型 str/int/float/bool/json |
| 向已有标签追加字段 | `add_tag_fields(tag, fields)`（旧名 `add_kind_fields`） | 玩法包 B 给**带该标签的实体**加字段（D18 起语义扩张） |
| 实例任意字段 | `set_data(entity_id, name, value)`（容器 = attrs，走原语分派） | buff 等临时效果，无需声明；未声明字段 UI 降级原始 JSON。同容器的 `set_attrs` 为合并写（不经分派），二选一 |

**分类标签（D10）**：`register_entity_kind(..., categories=("生物",))`——标签挂
分类（宽松，无需预注册）；**分类字段** `add_category_fields("生物", [hp])` 使该
分类全部标签获得字段（运行时合并，且在字段覆盖顺序里**排最后**）；
...
**两个层级（v0.3.0）**：
- **全局层**（代码加载）：enable/disable/uninstall 如上表——影响**所有世界**，
  管理端收在「世界 › 玩法包 › 高级：全局加载状态」里（高级操作，带说明）。
- **世界层**（激活集合）：`worlds.play_ids`——决定"这个世界有没有这个能力"，
  管理端是世界页里的**主开关**（`play_ids` 空 = 全部启用；「自定义」= 显式名单，
  切换时以当前已加载包生成名单）。两者共同决定某世界某能力是否可用：
  全局未加载 → 所有世界都没有；全局加载但世界未激活 → 只有该世界没有。

### 4.3.1 管理端导航（v0.3.0 世界上下文 + v0.4.0 地图总览）

```
通用管理            账户管理 / 邀请码 / 全部地图           ← 与世界无关（全部地图 = 跨世界总览）
世界 [下拉] ＋ ⚙     玩法包（本世界启停）/ 地图（本世界组织树）  ← 选中世界 = 下面的参考系
玩法包管理页（可收起） 各包注册的管理页（按当前世界激活集合过滤）
```

- 路由：`#/admin/accounts`、`#/admin/invites`、`#/admin/maps`（**跨世界全部地图总览**）、
  `#/admin/world/{world_id}/plays|maps`、`#/admin/maps/{map_id}`（地图编辑器）、
  `#/admin/pages/{play_id}/{key}`（管理页）
- 旧链接自动收敛（地址栏 `replaceState` 一并规范化）：`#/admin/plays` → 当前世界的玩法包、
  `#/admin/worlds` → `#/admin/maps`；世界里的话一律写成 `#/admin/world/{id}/{page}`
- 选中世界存 localStorage；地图编辑器打开时按地图归属回填世界上下文
- **地图治理（组织树/文件夹/地图归属）与世界概念相对独立**，可单独演进

### 4.3.2 地图治理（v0.4.0）

**组织树是纯管理维度**（像文件管理，不参与玩法逻辑），治理动作全部落在这三层：
世界 → 组织节点（文件夹，任意层级）→ 地图。

| 面 | 能力 |
|---|---|
| 组织树 | 递归任意层级渲染 + 折叠、**拖拽移动**（文件夹↔文件夹、地图↔文件夹）、**同级排序**（拖到行之间）、面包屑聚焦、内联重命名、按名字/地图 id 搜索 |
| 排序 | 文件夹与地图**共用一个序号空间**：每个容器内 `sort` 构成全序；新建/移入默认排到末尾；`POST /admin/worlds/{id}/reorder` 一次性绝对化整个容器的序号（拖拽落位走它） |
| 地图搬家 | `POST /admin/maps/{id}/move`：世界 / 组织节点 / 序号三者可任意组合；**跨世界自动落到世界根**（旧节点不属于新世界）；`world_id=null` = 解除归属 |
| 地图生命周期 | 列表内改名、**复制另存**（`POST /admin/maps/{id}/copy`：地块/连接/描述全量，同图目标重写为副本自己、跨图目标原样保留、身份化实体永不复制、可选带布景实体、归属跟随源地图并排到该节点末尾）、删除（身份化实体在场被拒，同 G2） |
| 删除文件夹 | 引擎拒绝删非空文件夹；管理端先提示"内容上移到上级节点"再执行（v0.3.0 的文案承诺"回到世界根"与引擎行为不符，属 bug，v0.4.0 修正为真实上浮） |
| 体检 | `GET /admin/lint[?map_id=]`：**死连接**（主目标不存在，运行时是**静默剔除**→ 必须提前告知）、意外目标失效、孤立地块（无进无出）、无出口/无入口、**出生点落空**、**实体悬空**、未归属世界、空地图；错误/提示两级 |
| 跨世界总览 | `#/admin/maps`：全部地图表格（世界 / 组织路径 / 地块 / 实体 / 可见性 / 体检/ 操作）、世界与孤儿过滤、搜索、**多选批量归属与转移** |
| 交互 | 管理端**不用原生 `confirm`/`prompt`/`alert`**（iframe/沙箱下会被浏览器屏蔽且风格脱节）——统一走站内确认层 `askConfirm()`（`webui/src/confirm.js` + `ConfirmHost.vue`）与内联编辑 |

- 体检是**纯数据检查**（无玩法语义），实现在引擎（`engine.lint_map` / `lint_maps`），
  管理端只是转发——将来 MCP 侧或玩法包也可直接用
- **地图编辑与玩法包解耦**：编辑器只认内核字段（位置/名字/描述/attrs/state），
  kind 未注册也能放置；玩法包只提供"名词表"（类型/模板/标签）做增强。
  实体声明体系见 §3.1（D18–D21），编辑器形态见 §4.3.3

### 4.3.3 地图编辑器（v0.5 / D23 / D24；v0.5.1 三视图）

编辑器 = **不依赖任何玩法包的世界数据编辑器**：玩法包装了就用它的"名词表"增强
体验（类型下拉、字段表单、能力预览），没装就退化为自由文本 + 原始 JSON，功能不缺。

**三个可切换视图**（v0.5.1：一次只专注一件事；`#/admin/maps/{id}[/entities|/templates]`
可深链，浏览器后退可用）。地图视图**不纵向滚动**：壳高 = 视口内，可视化画布吃掉
剩余空间，侧栏自己滚。

```
🗺 地图编辑 │ 🧍 实体 N │ 🧩 模板 N            ← 视图切换（带数量）
┌───────────────────────────────┬──────────────────┐
│ 可视化画布（缩放/平移/框选）     │ 地块面板（名称/描述/移动/删除/套模板）
│  滚轮缩放 · 拖空白平移 · 拖格子框选 │ 本格实体（这一格上的实体，可就地增删改）
│  适应窗口 / 1:1 / 平移模式        │ 出口面板（结构化：地图+坐标+权重+分时段文案）
└───────────────────────────────┴──────────────────┘
```

| 决策点 | 做法 |
|---|---|
| **不许静默丢字段**（G24） | 出口编辑是结构化的：目标 = 地图下拉 + 坐标 + 权重，文案 = 纯文本或分时段 JSON；**保存前一律弹差异预览**（旧值→新值），取消即不落盘 |
| **可视化面板**（v0.5.1） | 滚轮以光标为中心缩放（25%–250%）、拖空白平移、**平移模式**下可拖任意处平移（平移过不误触发点击）；「适应窗口」按内容尺寸缩放并居中（小图最多 150%，不把三格撑满屏）；网格右下多留一行一列 + 坐标输入框，包围盒之外与负坐标都能去 |
| **实体视图按规模设计**（v0.5.1） | 列表 = 搜索（名称/类型/标签/id）+ 类型/标签筛选 + 排序（位置/名称/类型）+ **分页**（50/100/200 每页）；行内快捷操作（定位/删除）；右侧**详情面板**（名称/描述/类型/标签/字段/JSON/合并能力/存为模板）——为"一张图上千实体"准备 |
| **地块级实体管理**（v0.5.1） | 地图视图选中地块 → 侧栏「本格实体」直接列出/删除/编辑；「在地块外管理 ›」跳到实体视图并**锁定该地块**（只看这一格）；实体列表「定位」回到地图并选中它所在格 |
| 实体数据编辑 | 按声明字段 schema 渲染控件（`FieldForm`），未声明字段走原始 JSON 兜底；`attrs`（玩法数据）与 `state`（动态状态）分开标注并提示 `block_move`/`invisible` 等内核约定键 |
| 标签组合 | `TagPicker`：勾选已声明标签 + 手输未注册标签（标红提示"无行为"）；类型预设标签以只读 chip 展示（D18） |
| 能力可视化 | 每个实体显示**合并后的能力**（挡路 / 动作并集 / 字段并集 / 未注册与未激活标签）——由服务端算（`GET /admin/maps/{id}` 的 `entities[].capabilities`，世界过滤后的真相），编辑器不自己重算 |
| 网格 | 多选（Ctrl）/框选（按住拖过格子）/出口方向箭头（简版可视化） |
| 复制粘贴 | 复制选中地块（存相对位移），粘贴到目标锚点：已占格跳过；同图出口按位移搬、跨图出口原样 |
| 移动地块 | 接 `engine.move_location`（内核早有）：原子重写自身坐标 + 全图指向它的连接 + 其上实体 |
| 模板 | 地块模板补上"套用"（相对偏移平移/跨图原样）；实体模板 = 包注册 + 本地，套用后可改 |
| 不做 | 撤销/重做（成本≈其余增强之和，由"保存前差异预览"兜底，D24） |
...
- **管理页 = 管理端独立路由页面**（`#/admin/pages/{play_id}/{key}`，非弹窗）——
  复杂配置玩法包可在页面内自建二级面板/分栏，避免弹窗套弹窗。
- **入口在管理端侧栏**（v0.2.1）：所有已注册管理页在侧栏自成一级分组
  「玩法包管理页」（可收起展开、状态本地记忆、当前页高亮），随注册表增删
  实时刷新（安装/启用/停用/卸载）；玩法包详情页不重复列出管理页，管理页
  自身页面也不放"返回玩法包"按钮——它就是管理端的一个导航页面。
  多个玩法包都注册管理页时，条目补显示所属包名。
...
- **落地示例**：items 包注册「物品管理」页（物品定义 list/create/update/delete，
  创建/更新即 `flush_item_defs` 落库）；管理端侧栏「玩法包管理页」分组下
  出现入口，点进去动态加载组件。
...
| D15 | 世界概念 | **世界 = 玩法包激活集合 + 数据边界**：worlds 表（play_ids 激活集合）+ 组织树（多层纯管理）+ maps 归属（world_id/folder_id）；玩法包全局加载一次、**按实体所在世界过滤分发（v0.3.0 起覆盖工具/视图/交互/事件/原语/kind/ui_hook 全路径，见 §3.0）**；身份全局、身份化实体数据跟人走（可跨世界跳转）；跳转 = `world_list`/`world_travel` 传送（**未实现**，见 GAPS G15）；play_data 按 (世界, 玩法包) 双层隔离 |
...
| D17 | 地图治理与编辑器边界 | **组织树 = 纯管理维度**（任意层级文件夹，不参与玩法逻辑）；**排序 = 容器内全序**（文件夹与地图共用一个序号空间，见 §4.3.2）；**地图编辑器不依赖任何玩法包**——只认内核字段，玩法包提供"名词表"做增强（类型/模板/标签，v0.5）；**体检归内核**（`lint_map` 纯数据检查，管理端只转发）；管理端**禁用原生 confirm/prompt/alert**（沙箱下被屏蔽），统一走站内确认层 |
| D18 | 实体标签体系 | **能力由标签承载**：实体 = 基底 `kind`（预设/标志位，必填）+ **实例级 `tags[]`（有序）**；`register_entity_kind(...)` 保留为**语法糖：注册一条隐式同名标签**（旧包零改动、全部既有测试零改动）；新增 `register_entity_tag(tag, ...)` 与 `register_entity_type(kind, tags=[...])`（**类型 = 标签预设**，即"A = {a,c}"）；`add_kind_fields` 语义扩张为"给**所有带该标签**的实体加字段"（修正 D9 里"B 包改 A 包命名空间"的别扭做法）；**类型与标签共用命名空间**（一套注册表、一套合并逻辑、一次冲突裁定） |
| D19 | 能力并集规则 | `block_move`：**任一标签为真即阻挡**（`state["block_move"]` 仍是最高的动态覆盖；标签写 false 想表达的是"对某些实体开放"，该用过滤器而非抵消别人的 true）；`interactions`：并集；`fields` 覆盖顺序**定死** = 隐式 kind 标签 → 实例 tags（数组序）→ 分类字段（越实例化越优先，可复现）；同名标签**后注册覆盖**（沿用 kind 行为，卸载即恢复，管理端可见来源）；**不做负标签**（要"去掉"就换 kind）；**允许未注册标签**（编辑器可手输、标注"未注册（无行为）"，实体照常存在——保证"编辑器不依赖玩法包"） |
| D20 | 身份判定（钉子①） | **只看基底 `kind`**（`IDENTITY_KINDS`），标签里出现 `player` **不改变身份**——"删标签绕过 D14 身份保护"的攻击面**因此不存在**，无需新造"内核保留标签"机制；NPC 升格为可登录角色必须经**身份服务更换基底 kind**（受控通道）；石化/附身/夺舍这类玩法由**玩法包自己的标签 + 过滤器**实现（身份是治理域、标签是玩法域） |
| D21 | 标签的能力面与激活过滤 | 能力解析**逐标签**按世界过滤：标签所属包在该世界未激活 → 该标签不参与并集（未注册标签无来源、无能力贡献）；规格级能力缓存以 `(kind, tuple(tags))` 为键，包加载/卸载时清空（不做到实体级） |
| D22 | 模板（地块 + 实体统一） | `templates` 表加 `scope`（`location` / `entity`），统一成一个"模板"概念（解决 v0.4.0 发现的地块模板与实体模板撞名）；**地块模板补上"套用"**（同图目标按相对偏移平移、跨图目标原样——语义早在模型注释里定好，一直没实现）；**实体模板 = 玩法包注册（内存，随包卸载消失）+ 管理端本地模板（落库）**，合并成一个选择列表；负载 = `{kind, tags[], name, desc, attrs, state}`；**不做多实体模板**（"房间模板"留待真实需求） |
| D23 | 编辑器的数据正确性（G24） | 编辑器**不得静默丢字段**：出口编辑改**结构化表单**（目标 = 地图 + 坐标 + 权重；文案 = 分时段条目），保存前给 **diff 预览**；实体数据**按声明字段 schema 渲染控件、未声明字段走原始 JSON 兜底**；显示**合并后的最终能力**（挡路/动作/字段）作为诊断；`attrs`（玩法数据）与 `state`（动态状态）分开标注 |
| D24 | 编辑器形态与增强 | 拆成**地块 / 实体 / 出口 / 模板四个面板**（不再单一 800 行组件）；做**地块多选 + 框选复制粘贴**、**出口方向箭头可视化（简版）**、接入 `engine.move_location`（内核早有、一直没入口）；**不做撤销/重做**（成本≈其余三项之和，由"保存前 diff 预览"兜底） |
| D25 | 死代码清理 | 删除 `EntityKindSpec.tick`（`register_entity_kind(tick=…)` 写了从没被读过、`list_kinds()` 也不返回、管理端不可见）；`list_kinds` 扩为"类型 + 标签"清单，标注**来源包**与**是否隐式** |
