...
### 4.3.1 管理端导航（v0.3.0：世界上下文）

```
通用管理            账户管理 / 邀请码                  ← 与世界无关
世界 [下拉] ＋ ⚙     玩法包（本世界启停）/ 地图（本世界）  ← 选中世界 = 下面的参考系
玩法包管理页（可收起） 各包注册的管理页（按当前世界激活集合过滤）
```

- 路由带世界：`#/admin/world/{world_id}/plays`、`#/admin/world/{world_id}/maps`、
  `#/admin/maps/{map_id}`（地图编辑器）、`#/admin/pages/{play_id}/{key}`（管理页）；
  旧链接 `#/admin/plays`、`#/admin/worlds` 自动收敛到当前世界的对应页面
