...
### 4.3.3 地图编辑器（v0.5 / D23 / D24）

编辑器 = **不依赖任何玩法包的世界数据编辑器**：玩法包装了就用它的"名词表"增强
体验（类型下拉、字段表单、能力预览），没装就退化为自由文本 + 原始 JSON，功能不缺。

```
┌ 地图网格（多选/框选/方向箭头） ┬ 地块面板（名称/描述/移动/删除/套模板）
│                                └ 出口面板（结构化：地图+坐标+权重+分时段文案）
├ 实体面板（类型 + 标签组合 → 按声明渲染字段 + 未声明字段原始 JSON + 合并能力预览）
└ 模板面板（地块 / 实体两类；玩法包注册 + 管理端本地）
```
...
| 能力可视化 | 每个实体显示**合并后的能力**（挡路 / 动作并集 / 字段并集 / 未注册与未激活标签）——服务端算（世界过滤后的真相） |
| 网格 | 多选（Ctrl）/框选（按住拖过格子）/出口方向箭头（简版可视化）；右下多留一行一列 + 坐标输入框，包围盒之外也能去 |
