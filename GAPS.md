...
- **地块模板是"半成品死重"**：`templates` 表 + 管理端只能建/删，
  **没有任何"套用模板"的入口**——`WorldTemplate` 的模型注释描述了
  "同图目标存方向相对偏移、放置时平移"的语义，引擎与管理端都没实现。
  与 v0.5 的**实体模板**还同名冲突，需一并决定：补 apply / 改名让位 / 废弃。
- **实体数据在管理端完全改不了**：编辑器只能改名字与删除，
  `PATCH /admin/entities/{id}` 虽然收 `attrs`/`state`，但没有 UI——
  这正是 v0.5「按声明渲染字段表单 + 原始 JSON 兜底」要解决的。
- **编辑器实体面板形态落后**：全图平铺一张表、无 kind 过滤、
  位置列还是冗余的 `map_id (row, col)`（都在同一张图上），实体一多没法用。
- **跨世界总览的批量搬家不报进度**：顺序发 N 个请求，中途失败只弹第一条错误，
  已成功的那几张已经改完了（无回滚也无"已完成 k/N"）。
- **总览页不能新建地图**（建图只能在世界页），跨世界视角下这步很别扭。
- **组织树只有拖拽排序**：没有 ▲▼ / 键盘替代（可访问性），无多选，无"复制文件夹"。
- **`GET /admin/lint` 每次全量算所有地图**：世界页其实只要本世界的，
  地图多时是白算（可加 `?world_id=` 过滤）。
- **升级后既有世界的"文件夹内地图顺序"会变成按名称**：旧库所有行 `sort` 都是 0
  （v0.4.0 之前没有这列），并列时按名称排；原先地图是按归属先后（插入序）显示的。
  属一次性观感变化，新建/拖拽过的世界不受影响。
- **状态**：📌 已记录（v0.5 开工时按此清单挑，不必全做）
